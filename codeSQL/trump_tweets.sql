...
insert overwrite local directory 'trump.retweet.long-nov8'
...
where (
	...
	(year <= 2015 or (year = 2016 and month < 12)
	  or (year = 2016 and month = 12 and day <= 31) ) 
and 
	(	
		(lower(text) like "%rt @realdonaldtrump%") 
		or (lower(retweeted_status.user.screen_name) like "%realdonaldtrump%")
		or (lower(user.screen_name) = 'realdonaldtrump')
	) );
