...
k = 20 
U <- svd_L$u[,1:k]
V <- svd_L$v[,1:k] #%*%Diagonal(k, svd_L$d[1:k]^(1/2))
U <- rbind(U,V)
rowN <- unlist(apply(U, MARGIN = 1, function(x) sqrt(sum(x*x)+1e-8)))
U1 <- Diagonal(length(rowN), rowN^(-1))%*%U
km_row2 = kmeans(U1, k, nstart = 500, iter.max =50)
...
Z1 <- matrix(0, n, k); Z2 <- matrix(0, m, k)
for (i in 1:k){
  Z1[which(km_row$cluster[1:n] == i), i] <- 1
  Z2[ which(km_row$cluster[(n+1):(n+m)] == i), i] <- 1
}


S <- t(Z1) %*% L %*% Z2
S1 <- Diagonal(k,colSums(Z1)^(-1))%*% S %*% Diagonal(k, colSums(Z2)^(-1))
image(S)
image(S1)
...
scores <- matrix(0, top, k)
for(i in 1:k){
  c_i1 <- colMeans( L[which(km_row$cluster[1:n]==i), ] )
  c_i2 <- colMeans( L[which(km_row$cluster[1:n]!=i), ] )
  #variance stablization transformation
  c_i <- sqrt(c_i1) - sqrt(c_i2)
  names_tmp <- name2[order(-c_i)]
  c_i <- c_i[order(-c_i)]
...
setkey(keyfriends_info, screen_name)
result <- cbind(clustering,keyfriends_info[clustering$screen_name])
write.csv(result, file ="../data/followers_Network/result_bip_k20_n.csv")




rm(L5, svd_L5, km_row2)
save (list =ls(), file = "../output/result_bip_L5.RData")
