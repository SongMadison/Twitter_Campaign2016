...
#June 17, 2018
1, try to run R in jupyter notebook, not successful yet.
...
# this will install 
#see https://github.com/ContinuumIO/anaconda-issues/issues/1133
#conda update -c r r-essentials


#April 28, 2018

### relationships, total occurences
-rw-rw-r-- 1 songwang songwang 7.7G Apr 29 02:31 edgelist_retweet.csv
-rw-rw-r-- 1 songwang songwang 986M Apr 29 02:40 edgelist_quoted.csv
-rw-rw-r-- 1 songwang songwang 6.3G Apr 29 02:49 edgelist_replied.csv
peach:relationships$ for file in *; do wc -l $file ; done
21,067,763 edgelist_quoted.csv
103,110,081 edgelist_replied.csv
217,543,781 edgelist_retweet.csv

### create the relationship edge list among users based on following, retweeting, replying

21067763 edgelist_quoted.csv
103110081 edgelist_replied.csv
217543781 edgelist_retweet.csv
21067763 el_users_quoted.csv
217543781 el_users_retweet.csv
103110081 el_user_sreplied.csv

total 31G
-rw-rw-r-- 1 songwang songwang 7.7G Apr 29 02:31 edgelist_retweet.csv
-rw-rw-r-- 1 songwang songwang 6.3G Apr 29 02:49 edgelist_replied.csv
-rw-rw-r-- 1 songwang songwang 986M Apr 29 04:07 edgelist_quoted.csv
-rw-rw-r-- 1 songwang songwang 8.3G Apr 29 13:15 el_users_retweet.csv
-rw-rw-r-- 1 songwang songwang 6.2G Apr 29 13:16 el_users_replied.csv
-rw-rw-r-- 1 songwang songwang 959M Apr 29 13:46 el_users_quoted.csv



### next step, we need to do construct the retweeting graph 
#### - among users period 1, 2, 3
#### - remove certain users both columnwise and row-wise


need to narrow down to people and 


#April 23, 2018
"/data/SongWang/Twitter_Campaign2016/data/friends_info/edgelist_Feb27/RData":
The A1.Rdata, A2.Rdata, A3.RData is a little bit different from those in 17-1-prepData-followingGraph.R
```{r}
load("A1.RData")
dim(A)
#[1]  55533 578739
...
#    10.0    106.0    276.0    742.2    717.0 188703.0 
dc = colSums(A)
summary(dc)
#    Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
#    8.00    18.00    32.00    71.22    67.00 50993.00 
load("A2.RData")
dim(A)
#[1] 161493 714074
dr = rowSums(A)
summary(dr)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   10.0   120.0   308.0   691.4   781.0 95683.0 
dc = colSums(A)
summary(dc)
#    Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
#     9.0     24.0     53.0    156.4    132.0 155147.0 
...
summary(dr)
#    Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
#    10.0    256.0    571.0    943.3   1083.0 114549.0 
dc = colSums(A)
summary(dc)
#    Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
#     9.0     22.0     53.0    170.8    137.0 103396.0
```

# April 22, 2018
## To use the most up-to-date cluster labels, and also daily_counts is across all the tweets, 636.7 millions tweets
```{r}
df1 <- read.csv("../combined_data/Trump_followers_labels_6_18.csv")
df2 <- read.csv("../combined_data/followers_with_cluster_info.csv", colClasses = c("character"))
# df2$clust_cat[df2$clust_cat=="non-political"]="apolitical"
# df2$clust_cat[df2$clust_cat=="Trump supporters-alt right"] = "far right conservatives"
# df2$clust_cat[df2$clust_cat=="conservatives"] = "mainstream conservatives"

sort(unique(df1$cluster_type))
sort(unique(df2$clust_cat))
names(df2)

as_str = c("period","cluster", "clust_label","clust_cat","id_str","screen_name", "name", "description","created_at",
           "lang", "location","time_zone", "verified","first_date_timeline","last_date_timeline")
idx <- match(as_str, colnames(df2))
for (i in 1:ncol(df2)){
    if (i %in% idx){ 
        next
    }else{
...
write.csv(df2, file = "../combined_data/followers_with_cluster_info.csv", row.names = F)



dailycounts <- read.csv("../combined_data/daily_counts.csv")
sum(dailycounts$count)
#636719564
...
#July 2, 2017
Find another problem in the downloding the profile info of the 13million users
I found that somehow that the script did not download first 22512 ids. The sad thing is that I lost the original
json file. I have no clue what went wrong now.  -- '5346104/data/SongWang/Twitter_Campaign2016/data/followers_info/ids/missing_22k.txt

downloaded the new jsons in case

```{r}
library(smappR)
ids_22k <- readLines('/data/SongWang/Twitter_Campaign2016/data/followers_info/ids/missing_22k.txt')
users <- getUsersBatch(ids =ids_22k,oauth_folder = './credentials/credential_mixed01/',output = '../data/followers_info/jsons/missing_22k.jsons',random = F )

#22399 out of 22512 are successfully downloaded, while the other have following error :$errors
#$errors[[1]]
#$errors[[1]]$code
#[1] 17

#$errors[[1]]$message
#1] "No user matches for specified terms."

#experiment with downloading the friends list

output <- paste0("../data/followers_info/csvs/missing_22k_friends_list_", Sys.Date(),".txt")
my_oauth_folder <- "./credentials/credential_mixed02/" #91 tokens
SN1 = users$screen_name
...
calculate the successful rate for sampleFeb17, sampleMar20. friendslist
We found that sampleFeb71 failure rate was much higher
```{r}
library(doParallel)
...
sampFeb17 <- read.csv("../data/friends_info/edgelist_Feb27/originalData/sampleFeb17.csv")
idx1 <- match(sampFeb17$non.random, sns)
indicator1 <- unlist( lapply( adj_list[idx1], FUN = function(x) (length(unlist(x))==2)&& unlist(x)[2] == 'NA') )
sampFeb17$friends_list = unlist(indicator1)

sampMar20 <- read.csv("../data/friends_info/edgelist_Feb27/originalData/sampleMar20.csv")
idx2 <- match(sampMar20$screen_name, sns)
indicator2 <- unlist( lapply( adj_list[idx2], FUN = function(x) (length(unlist(x))==2)&& unlist(x)[2] == 'NA') )

sampFeb17$friends_failed = unlist(indicator1)
sampMar20$friends_failed = unlist(indicator2)

all_samp <- read.csv("../data/friends_info/edgelist_Feb27/all_samp_info.csv")
library(tidyverse)
sampFeb17 <- sampFeb17 %>% left_join(all_samp, by = c("non.random" = "screen_name"))
lm1 <- lm(followers_count~as.factor(friends_failed)-1, sampFeb17)
coef(lm1)
#as.factor(friends_failed)FALSE  as.factor(friends_failed)TRUE 
#                      8775.612                      19065.540 

#sampMar20 <- sampMar20 %>% left_join(all_samp, by ="screen_name")
lm2 <- lm(followers_count~as.factor(friends_failed)-1, sampMar20)
coef(lm2)
#as.factor(friends_failed)FALSE  as.factor(friends_failed)TRUE 
#                     10304.405                       3784.891 
```
...
=======
# ```{r, eval= FALSE}
>>>>>>> 83cf97e34a6708af9bb1eaa0a6517390142ae824
